...
public class VendorRestServiceImpl implements VendorRestService {
	...
	public List<User> getAllVendors() {
		...
		ResponseEntity<User[]> responseEntity = restTemplate.exchange(serviceUrl, HttpMethod.GET, httpEntity,
				User[].class);
	...
	public User getVendor(int id) {
		RestTemplate restTemplate = restHelper.getRestTemplate();
		HttpEntity httpEntity = new HttpEntity(restHelper.getHttpHeaders());
		ResponseEntity<User> responseEntity = restTemplate.exchange(serviceUrlExtended + id, HttpMethod.GET, httpEntity,
				User.class);
		User vendor = responseEntity.getBody();
		return vendor;
	}
