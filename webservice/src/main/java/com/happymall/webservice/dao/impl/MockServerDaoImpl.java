...
public class MockServerDaoImpl extends GenericDaoImpl<MockServer> implements MockServerDao {
	...
	public Boolean isCardValid(CardDetail cardDetail) {
		...
		if (!M.getActiveInd().equalsIgnoreCase("A"))
			return false;
		if (M.getCardNumber().equals(cardDetail.getCardNumber())
				&& M.getNameOnCard().equalsIgnoreCase(cardDetail.getNameOnCard())
				&& M.getCvv().equalsIgnoreCase(cardDetail.getCvv()))
			return true;
		return false;
	}
